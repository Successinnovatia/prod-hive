...
const StakeholderMapper: React.FC<StakeholderMapperProps> = ({ isOpen, onClose }) => {
  ...
  const relationships = [
    { value: 'strong', label: 'Strong', color: 'bg-success/15 text-success' },
    { value: 'moderate', label: 'Moderate', color: 'bg-warning/15 text-warning' },
  ...
  const getQuadrant = (influence: number, interest: number) => {
    if (influence >= 7 && interest >= 7) return { name: 'Manage Closely', color: 'bg-red-100 border-destructive' };
    if (influence >= 7 && interest < 7) return { name: 'Keep Satisfied', color: 'bg-warning/15 border-yellow-300' };
    if (influence < 7 && interest >= 7) return { name: 'Keep Informed', color: 'bg-primary/10 border-blue-300' };
    return { name: 'Monitor', color: 'bg-muted border-input' };
  };
      ...
      <div className="bg-card rounded-xl max-w-6xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-border">
          <div className="flex items-center">
            ...
            <h2 className="text-xl font-bold text-foreground">Stakeholder Mapping Tool</h2>
          </div>
        ...
        <div className="p-6">
          ...
          <div className="bg-muted p-4 rounded-lg mb-6">
            <h3 className="text-lg font-semibold text-foreground mb-4">Add New Stakeholder</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <input
                ...
                className="px-3 py-2 border border-input rounded-lg focus:ring-2 focus:ring-ring focus:border-ring"
              />
              <input
                type="text"
            ...
            <button
              ...
              className="mt-4 bg-primary text-primary-foreground px-4 py-2 rounded-lg hover:bg-primary/90 transition-colors duration-200 flex items-center space-x-2"
            >
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-foreground">Influence-Interest Matrix</h3>
              <button
            ...
            <div className="grid grid-cols-2 gap-4 h-96">
              ...
              <div className="bg-destructive/10 border-2 border-red-200 rounded-lg p-4">
                <h4 className="font-semibold text-red-800 mb-2">Manage Closely</h4>
                <p className="text-xs text-destructive mb-3">High Influence, High Interest</p>
                <div className="space-y-2">
                  {stakeholders.filter(s => s.influence >= 7 && s.interest >= 7).map(stakeholder => (
                    <div key={stakeholder.id} className="bg-card p-2 rounded border text-xs">
              ...
              <div className="bg-yellow-50 border-2 border-yellow-200 rounded-lg p-4">
                <h4 className="font-semibold text-warning mb-2">Keep Satisfied</h4>
              ...
              <div className="bg-blue-50 border-2 border-primary/30 rounded-lg p-4">
                <h4 className="font-semibold text-primary mb-2">Keep Informed</h4>
                <p className="text-xs text-primary mb-3">Low Influence, High Interest</p>
                <div className="space-y-2">
                  {stakeholders.filter(s => s.influence < 7 && s.interest >= 7).map(stakeholder => (
                    <div key={stakeholder.id} className="bg-card p-2 rounded border text-xs">
                      <div className="font-medium">{stakeholder.name}</div>
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-foreground">Detailed Stakeholder Information</h3>
            {stakeholders.map(stakeholder => {
              const quadrant = getQuadrant(stakeholder.influence, stakeholder.interest);
              const relationship = relationships.find(r => r.value === stakeholder.relationship);
              
              return (
                <div key={stakeholder.id} className="bg-card border border-border rounded-lg p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <h4 className="text-lg font-semibold text-foreground">{stakeholder.name}</h4>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3">
                    <div>
                      <div className="text-sm font-medium text-foreground">Influence: {stakeholder.influence}/10</div>
                    ...
                    <div>
                      <div className="text-sm font-medium text-foreground">Interest: {stakeholder.interest}/10</div>
                      <div className="w-full bg-muted rounded-full h-2">
                        <div 
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div>
                      <div className="font-medium text-foreground mb-1">Cultural Context:</div>
                      <div className="text-muted-foreground">{stakeholder.culturalContext}</div>
                    </div>
                    <div>
                      <div className="font-medium text-foreground mb-1">Communication Preference:</div>
                      <div className="text-muted-foreground">{stakeholder.communicationPreference}</div>
                    </div>
                  </div>
                </div>
              );
